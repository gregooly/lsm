...
public class CollectionsController : TenantScopedController
{
    private readonly LaundryMsDbContext _dbContext;

    public CollectionsController(LaundryMsDbContext dbContext)
    {
    ...
    public async Task<IActionResult> Index(CancellationToken cancellationToken)
    {
        ...
        var jobs = await LogisticsJobsListLoader.LoadAsync(
            _dbContext,
            cancellationToken,
            q => q.WhereCollectionJobType().Where(x => x.CustomerId == customerId));
    ...
    public async Task<IActionResult> Details(ulong id, CancellationToken cancellationToken)
    {
        ...
        var expectedItems = await _dbContext.JobExpectedItems
            ...
            .Select(x => new CollectionExpectedItemRowViewModel
            {
        ...
        var recentScans = await _dbContext.LinenMovementEvents
            ...
            .Take(400)
            .Select(e => new CollectionScanEventRowViewModel
            {
                EventId = e.Id,
        ...
        var vm = new CollectionJobDetailViewModel
        {
            Id = head.Id,
    ...
    public async Task<IActionResult> Finalize(ulong id, CancellationToken cancellationToken)
    {
        ...
        if (entity == null || !IsAllowedCollectionType(entity.JobType))
            return NotFound();
        ...
        TempData["CollectionsToast"] = "Job marked complete.";
    ...
    public async Task<IActionResult> StartProgress(ulong id, CancellationToken cancellationToken)
    {
        ...
        TempData["CollectionsToast"] = "Job started.";
        return RedirectToAction(nameof(Details), new { id });
    }
    ...
    public async Task<IActionResult> Create(LogisticsJobFormViewModel model, CancellationToken cancellationToken)
    {
        ...
        if (!IsAllowedCollectionType(model.JobType))
            return Json(new { ok = false, errors = new Dictionary<string, string[]> { [nameof(LogisticsJobFormViewModel.JobType)] = ["Job type must be collection, pickup, or collect."] } });
    ...
    public async Task<IActionResult> Edit(LogisticsJobFormViewModel model, CancellationToken cancellationToken)
    {
        ...
        if (!IsAllowedCollectionType(entity.JobType))
            return Json(new { ok = false, message = "Job is not a collection job." });
    ...
    {
        model.JobType = string.IsNullOrWhiteSpace(model.JobType) ? "collection" : model.JobType.Trim().ToLowerInvariant();
    ...
    private static bool IsAllowedCollectionType(string? jobType)
    {
        if (string.IsNullOrWhiteSpace(jobType)) return false;
        var t = jobType.Trim().ToLowerInvariant();
        return t is "collection" or "pickup" or "collect";
    }
